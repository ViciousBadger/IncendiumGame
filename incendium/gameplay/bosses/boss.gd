...
func create_part(id, pos, layer, index, parentsides):
	...
	if alive || id == "":
		...
		part.connect("dead", self, "_on_bosspart_dead")
...
func _on_bosspart_dead():
	if dead_map.size() == part_map.size():
		# Boss is a goner
		kill()